...
from src.data.utils import get_pred_df, get_truth_df, restore_prefix
...
def evaluate_metric(
    ...
    """
    truth_df = truth_df.join(pred_df, on=["document", "token_index"], how="left").with_columns(
...
def get_best_negative_individual_threshold(
    ...
    pred_df = oof_df.with_columns(
        positive_pred=np.argmax(
            oof_df.select(
                [col for col in oof_df.columns if "pred" in col and "pred_0" != col]
            ).to_numpy(),
            axis=1,
        )
        + 1,
    ...
    for pii_type, pii_pred_df in tqdm(
        pred_df.group_by(["positive_pred"]), total=pred_df["positive_pred"].n_unique()
    ):
        best_score = 0
        for th in tqdm(np.arange(lower_th, upper_th, 0.01)):
            ...
            # 後で共通化する
            tp = pii_pred_df.filter(
                (pl.col("label") != 0) & (pl.col("label") == pl.col("pred"))
            ).shape[0]
...
def check_data_validity_2nd(train_data: list[dict], use_fn_label: bool):
    ...
    for data in train_data:
        ...
        for label, pred, second_label in zip(
            data["labels"], data["name_preds"], data["second_labels"]
        ):
