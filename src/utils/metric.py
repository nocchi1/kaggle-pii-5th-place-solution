...
from src.data.utils import get_truth_df, get_pred_df, restore_prefix
...
def evaluate_metric(
    ...
    Args:
        pred_df: necessary_columns = ['document', 'token_index', 'pred']
        truth_df: necessary_columns = ['document', 'token_index', 'label']
    """
    truth_df = (
        truth_df.join(pred_df, on=['document', 'token_index'], how='left')
        .with_columns(pred = pl.col('pred').fill_null(0))
    )
    tp = truth_df.filter((pl.col('label') != 0) & (pl.col('label') == pl.col('pred'))).shape[0]
    fp = truth_df.filter((pl.col('label') == 0) & (pl.col('pred') != 0)).shape[0]
    fn = truth_df.filter((pl.col('label') != 0) & (pl.col('pred') == 0)).shape[0]
    fp_fn = truth_df.filter((pl.col('label') != 0) & (pl.col('pred') != 0) & (pl.col('label') != pl.col('pred'))).shape[0]
    score = calculate_fbeta(tp, fp + fp_fn, fn + fp_fn)
    return score


def get_best_negative_threshold(config, oof_df: pl.DataFrame):
    truth_df = get_truth_df(
        config, 
        oof_df['document'].unique().to_list(), 
        is_label_idx=True
    )
    best_score = 0
...
def get_best_negative_individual_threshold(config, oof_df: pl.DataFrame) -> Dict[int, float]: # ここのpii_typeごとのthresholdを返す
    ...
    for pii_type, pii_pred_df in tqdm(pred_df.group_by(['positive_pred']), total=pred_df['positive_pred'].n_unique()):
        ...
        for th in tqdm(np.arange(lower_th, upper_th, 0.01)):
            ...
            fp_fn = pii_pred_df.filter((pl.col('label') != 0) & (pl.col('pred') != 0) & (pl.col('label') != pl.col('pred'))).shape[0]
            score = calculate_fbeta(tp, fp + fp_fn, fn + fp_fn)
            if score > best_score:
                best_score = score
                best_th = th
...
def get_best_threshold_2nd(config, pred_df: pl.DataFrame):
    truth_df = get_truth_df(
        config, 
        pred_df['document'].unique().to_list(), 
        is_label_idx=True
    )
    best_score = 0
